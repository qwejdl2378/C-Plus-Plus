/**
 * @file
 * @brief Solve a multivariable first-order ordinary differential equation (ODE) using [Semi-implicit Euler method](https://en.wikipedia.org/wiki/Semi-implicit_Euler_method) (半隐式欧拉法求解多变量一阶常微分方程)
 *
 * @details
 * 半隐式欧拉法（Semi-implicit Euler，又称辛欧拉法 Symplectic Euler）是一种针对哈密顿系统（如简谐振子、天体轨道动力学）的辛积分方法。
 * 它是把时域位置用显式欧拉更新，而速度（或动量）则采用隐式欧拉，即用更新后的最新位置值进行更新：
 *
 * $v_{n+1} = v_n + dx \cdot g(t_n, u_n)$
 * $u_{n+1} = u_n + dx \cdot f(t_n, v_{n+1})$
 *
 * 在本例中，求解的二阶谐振子方程为：
 * $\dot{u} = v$
 * $\dot{v} = -\omega^2 u$
 ...
 * 2. **遗留 CSV 垃圾文件污染 Bug**：
 *    原程序运行后会直接在目录下遗留 `semi_implicit_euler.csv` 和 `exact.csv`。
 *    **修复**：添加文件自动清理逻辑，在测试验证完毕后通过 `std::remove` 自动删除生成的临时 CSV 文件。
 * 3. **缺失单元测试校验断言**：
 *    **修复**：半隐式欧拉法是一阶辛算法，具有长期能量守恒性。在 $dx = 0.01$ 时，其在 $t=10.0$ 的累计轨道误差极小，
 *    增加对最终估计解与解析解的高精度断言校验：误差上限设定为 `0.02`。
 *
 * @see ode_forward_euler.cpp, ode_midpoint_euler.cpp
...
/**
 * @brief 执行单步半隐式欧拉计算 (辛欧拉)
 */
void semi_implicit_euler_step(const double dx, const double &x,
                              std::valarray<double> *y,
                              std::valarray<double> *dy) {
    problem(x, y, dy);         
    y[0][0] += dx * dy[0][0];  // 先显式更新位置 u
    problem(x, y, dy);         // 基于更新后的位置计算新的速度导数
...
double semi_implicit_euler(double dx, double x0, double x_max,
    ...
    if (save_to_file) {
        ...
        if (!fp.is_open()) {
            std::perror("Error opening semi_implicit_euler.csv ");
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    // 计算半隐式欧拉法近似解
    double total_time = semi_implicit_euler(step_size, X0, X_MAX, &Y_approx, true);
    std::cout << "\tSemi-implicit Euler computation time = " << total_time << " s\n";

    // 保存精确解析值
    save_exact_solution(X0, X_MAX, step_size, Y0);

    // 核心修复：高精度单元测试校验，由于半隐式法为辛算法，性质良好，误差上限设为 0.02
    ...
    assert(std::abs(Y_approx[0] - exact_u) < 0.02);
    assert(std::abs(Y_approx[1] - exact_v) < 0.02);
    std::cout << "Semi-implicit Euler ODE solver assertions passed!\n";
